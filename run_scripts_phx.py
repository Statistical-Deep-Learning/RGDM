...
emb_dir = "/data/yyang409/changyu/generate_robust_graph_embedding"
...
vae_dir_1 = f"{model_dir}/gge/robustgraphvae_{args.dataset}_{args.att_type}_{args.att_rate}_only_feat_lr{lr_str}"
vae_dir_2 = f"{model_dir}/gge/robustgraphvae_{args.dataset}_{args.att_type}_{args.att_rate}_freeze_enc_feat_map_lr{lr_str}"
...
diffusion_batch_size = 8192

#result dirs
vae_decode_feature = f"{vae_dir_3}/{args.dataset}_latents_{genum}_robustgvae_250_64_decode_feat.npy"
vae_decode_map = f"{vae_dir_3}/{args.dataset}_latents_{genum}_robustgvae_250_64_decode_map.npy"


python_path = "/data/yyang409/changyu/graph_diffusion/bin/python"

def command(args):
    script_vae_1 = f'CUDA_VISIBLE_DEVICES={args.gpu_id}  {python_path}  {emb_dir}/train_robust_graph_vae.py --moddir {vae_dir_1} --samdir {vae_dir_1} --datadir {attk_embd_dir} --labeldir {attk_label_dir} --adjdir {attk_adj_dir} --norm 1 --lr {lr_vae} --coef_recon 1 --coef_map 0 --factor 0  --factor_edgemap 0 --interval {save_interval} --intervalplot 10 --epoch {epoch_1_vae} --freeze 0 --dataset {args.dataset} --neighbor_map_dim {datalen} --batchsize {datalen} --feat_emb_dim {feat_emb_dim}'

    script_vae_2 = f'CUDA_VISIBLE_DEVICES={args.gpu_id} {python_path}  {emb_dir}/train_robust_graph_vae.py --moddir {vae_dir_2} --samdir {vae_dir_2} --datadir {attk_embd_dir} --labeldir {attk_label_dir} --adjdir {attk_adj_dir} --checkpoint_path {vae_dir_1}/ckpt_{epoch_1_vae}_checkpoint.pt --norm 1 --lr {lr_vae} --coef_recon 1 --coef_map 1 --factor 0  --factor_edgemap 0 --interval {save_interval} --intervalplot 10 --epoch {epoch_2_vae} --freeze 1 --dataset {args.dataset} --neighbor_map_dim {datalen} --batchsize {datalen} --feat_emb_dim {feat_emb_dim}'
 
    script_vae_3 = f'CUDA_VISIBLE_DEVICES={args.gpu_id} {python_path}  {emb_dir}/train_robust_graph_vae.py --moddir {vae_dir_3} --samdir {vae_dir_3} --datadir {attk_embd_dir} --labeldir {attk_label_dir} --adjdir {attk_adj_dir} --checkpoint_path {vae_dir_2}/ckpt_{epoch_2_vae}_checkpoint.pt --norm 1 --lr 1e-5 --coef_recon 1 --coef_map 1 --interval 2 --intervalplot 2 --epoch 300 --freeze 1 --dataset {args.dataset} --neighbor_map_dim {datalen} --batchsize {datalen} --factor {factor_edgefeat} --factor_edgemap {factor_edgemap} --feat_emb_dim {feat_emb_dim}'
    ...
    script_diffusion = f'CUDA_VISIBLE_DEVICES={args.gpu_id} {python_path}  {emb_dir}/train.py --batchsize {diffusion_batch_size} --modch 64 --moddir {diffusion_dir} --samdir {diffusion_dir} --epoch 3000 --interval 500 --intervalplot 500 --nettype unet_1d --inch 1 --outch 1 --inputsize 64 --clsnum {clsnum} --datatype gclemb --datadir {vae_encode_data} --labeldir {attk_label_dir} --genum {clsnum*80} --genbatch {clsnum*40} --norm 1'
