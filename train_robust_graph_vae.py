    ...
    with tqdm(dataloader, dynamic_ncols=True) as tqdmDataLoader:
        for feature_emb_gt, normalized_adj, neighbor_map_gt, labels,  positional_embeddings, _ in tqdmDataLoader:
            ...
            for epc in range(lastepc, params.epoch):
                ...
                #edge_loss = F.mse_loss(torch.sigmoid(torch.matmul(z, z.T)), neighbor_map_gt*edge_mask, reduction='mean')
                #edge_loss = F.mse_loss(torch.sigmoid(torch.matmul(reconstructed_feat, reconstructed_feat.T)), neighbor_map_gt*edge_mask, reduction='mean')
                #edge_loss = F.binary_cross_entropy(torch.sigmoid(torch.matmul(norm_reconstructed_feat, norm_reconstructed_feat.T)), neighbor_map_gt*edge_mask, reduction='mean')
                ...
                #factor = 0.99999
                #loss = params.coef_recon * (l2_loss + (1-factor) * edge_loss )+ params.coef_map * bce_loss 
                loss = params.coef_recon * l2_loss + params.factor * edge_loss+ params.coef_map * bce_loss  + params.factor_edgemap * edge_loss_map
                loss.backward()
                optimizer.step()
...
def encode(params:argparse.Namespace):
    ...
    #save data
    np.save(os.path.join(params.samdir, f'{params.dataset}_latents_{latents.shape[0]}_robustgvae_{params.lastepo}_{latents.shape[-1]}_encode.npy'),latents)
